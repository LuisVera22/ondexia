package com.ondexia.infrastructure.salida.persistencia.comun;

import javax.sql.DataSource;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.boot.ApplicationArguments;
import org.springframework.boot.ApplicationRunner;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Component;

/**
 * Impide arrancar si el rol de base de datos puede saltarse Row Level Security.
 *
 * <p>Un rol con {@code SUPERUSER} o {@code BYPASSRLS} <strong>no está sujeto a
 * ninguna política</strong>, y {@code FORCE ROW LEVEL SECURITY} tampoco le
 * afecta: {@code FORCE} solo alcanza al propietario de la tabla.
 *
 * <p>El resultado es el peor estado posible de un control de seguridad: las
 * políticas existen, {@code pg_policies} las lista, el código parece correcto y
 * no filtran absolutamente nada. Sin error y sin aviso.
 *
 * <p>No es hipotético — la imagen oficial de PostgreSQL crea
 * {@code POSTGRES_USER} como superusuario, así que el entorno de desarrollo por
 * omisión <em>tiene</em> el problema. Se descubrió porque las pruebas de
 * aislamiento fallaron; sin ellas habría llegado a producción pareciendo
 * protegido.
 */
@Component
public class ComprobacionAislamiento implements ApplicationRunner {

    private static final Logger LOG = LoggerFactory.getLogger(ComprobacionAislamiento.class);

    private final JdbcTemplate jdbc;

    public ComprobacionAislamiento(DataSource origenDatos) {
        this.jdbc = new JdbcTemplate(origenDatos);
    }

    @Override
    public void run(ApplicationArguments argumentos) {
        String rol = jdbc.queryForObject("select current_user", String.class);

        Boolean saltaPoliticas = jdbc.queryForObject(
                "select rolsuper or rolbypassrls from pg_roles where rolname = current_user",
                Boolean.class);

        if (Boolean.TRUE.equals(saltaPoliticas)) {
            throw new IllegalStateException("""

                    ==========================================================================
                      El rol de base de datos '%s' es SUPERUSER o tiene BYPASSRLS.

                      Row Level Security NO se aplica a esos roles, ni siquiera con FORCE. El
                      aislamiento multiempresa quedaria activo y sin ningun efecto: las
                      consultas devolverian datos de todos los clientes sin dar ningun error.

                      Se aborta el arranque a proposito.

                      En local:  no te conectes con POSTGRES_USER. Es el superusuario
                                 bootstrap del cluster y NO se le puede quitar el atributo.
                                 La aplicacion usa el rol 'ondexia', que crea
                                 docker/initdb/10-rol-aplicacion.sql. Si la base ya existia:
                                 docker compose down -v.
                      En AWS:    el usuario maestro de RDS no es el superusuario bootstrap,
                                 asi que esto no deberia ocurrir. Si ocurre, alguien
                                 concedio BYPASSRLS.
                    ==========================================================================
                    """.formatted(rol, rol));
        }

        Integer sinPolitica = jdbc.queryForObject("""
                select count(*)
                from pg_tables t
                where t.schemaname = 'public'
                  and exists (
                      select 1 from information_schema.columns c
                      where c.table_schema = t.schemaname
                        and c.table_name = t.tablename
                        and c.column_name = 'empresa_id')
                  and not exists (
                      select 1 from pg_policies p
                      where p.schemaname = t.schemaname and p.tablename = t.tablename)
                """, Integer.class);

        if (sinPolitica != null && sinPolitica > 0) {
            // Aviso y no error: hoy hay tres tablas legitimamente fuera de RLS
            // —empresa, sucursal y usuario_empresa son las que hay que leer PARA
            // saber cual es la empresa activa—. Convertirlo en error obligaria a
            // mantener una lista de excepciones que acabaria perdiendo sentido.
            LOG.warn("Hay {} tabla(s) con columna empresa_id sin politica de RLS. "
                    + "Si no es una de las de resolucion de contexto, falta llamar a "
                    + "activar_aislamiento_empresa() en su migracion.", sinPolitica);
        }

        LOG.info("Aislamiento multiempresa verificado: rol '{}' sujeto a las politicas.", rol);
    }
}

...
public final class TraductorRestricciones {
     ...
     */
    public static Optional<ConflictoException> traducir(DataIntegrityViolationException error) {
        String restriccion = nombreDeRestriccion(error);

        if (restriccion != null) {
            Traduccion conocida = CONOCIDAS.get(restriccion);
            if (conocida != null) {
                return Optional.of(new ConflictoException(conocida.codigo(), conocida.mensaje()));
            }
        }
        ...
        for (var entrada : POR_MENSAJE.entrySet()) {
            if (texto.contains(entrada.getKey())) {
                return Optional.of(new ConflictoException(
